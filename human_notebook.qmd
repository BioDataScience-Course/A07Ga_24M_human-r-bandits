...
On observe que le poids a tendance à augmenter avec le tour de taille. On peut également noter que le nombre d'observation est bien plus grand de 80 à 100cm de tour de taille.
...Vos deux descriptions ici...
...
biometry <- biometry %>%
  mutate(rth = case_when(
      sexe == "F" & rth < 0.65 ~ "Risque bas",
      sexe == "F" & rth >= 0.65 & rth <= 0.8 ~ "Normal",
      sexe == "F" & rth > 0.8 ~ "Obésité androïde",
      sexe == "H" & rth < 0.85 ~ "Risque bas",
      sexe == "H" & rth >= 0.85 & rth <= 0.95 ~ "Normal",
      sexe == "H" & rth > 0.95 ~ "Obésité androïde"))
```
...
```{r test_t_2}
t.test(data = biometry, masse_corr ~ bmi,
  alternative = "", conf.level = 0.95, var.equal = TRUE)
...
H0:μ jamais = μ rarement = μ régulièrement = μ souvent H1 : μ i =! μ j
...
H0: %\mu%\[vg\] = %\mu%\[carn\] == %\mu%\[om\] H1: ∃(i,j) tel que %\mu%\[i\] != %\mu%\[j\] (avec i et j sont "carnivore", "omnivore" ou "végétarien") <!--% Vérification des conditions d'application -->
...
Nous ne rejetons pas H0 au seuil alpha de 5%, nous ne pouvons donc pas signicativement dire que le Bmi dépend de l'alimentation des différents sujets mesurés lors de l'expérimentation.
