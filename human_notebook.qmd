...
chart(data = biometry, masse_real ~ tour_poignet | genre) + geom_point() + ylab("Masse de l'individu kg") + xlab("tout du poignet cm")
...
tableau_poignet <- biometry2 %>%
  select(tour_poignet, masse_real) %>%
  mutate(
  ...
  summarise(
    moyenne_masse_real = mean(masse_real, na.rm = TRUE),
    nb_observations = n()
  )
...
```{r plot_indice1}
chart(data = biometry, masse_real ~ wwr | genre) + geom_point() + ylab("Masse de l'individu kg") + xlab("rapport taille et poignet ")
...
```{r plot_chi2_1}
chart(data = biometry, masse_real ~ wwr | genre) + 
  geom_point() + 
