...
biometry %>.%
  ...
  ssummarise(.,
    `Nombre d'induvidus mesurés` = fn(bmi_cat)
  ) -> d
  as.data.frame(d)
...
```{r plot_chi2_1}
chart(data = biometry, rtt ~ bmi | genre) + 
  geom_point() + 
...
bartlett.test(data = biometry, log(rtt) ~ sucre)

anova(biometry_anova1_1 <- lm(data = biometry, log(rtt) ~ sucre)) |> tabularise()

```

```{r qqplot_anova1_1}
chart$qqplot(biometry_anova1_1, lang = "fr")
