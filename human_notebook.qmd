...
biometry <-
 ...
 breaks = c(0, 0.5, Inf),  # découpage
  labels = c("sasn risque", "à risque"),  # Noms des classes
  right = FALSE)
```
...
```{r plot_chi2_1}
chart(data = biometry, rtt_cat ~ bmi_cat2 %col=% bmi_cat2) + 
  geom_point() + 
...
tableau <- table(biometry$bmi_cat2, biometry$rtt_cat)
chisq.test(tableau)
```
...
```{r test_anova2_2}
biometry_anova2_2 <- lm(data = biometry, mean(bmi) ~ intolerance_lactose * intolerance_gluten)
anova(biometry_anova2_2)
