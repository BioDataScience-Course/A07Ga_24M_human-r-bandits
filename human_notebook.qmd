...
chart(data = biometry, masse_corr ~ tour_poignet | genre) + geom_point() 



```

On observe une tendance à avoir une masse plus importante lorsque le tour de poigne est lui aussi plus grand. On observe également que les hommes ont des poignets et une masse plus large en générale.

```{r description 2 étudiant 1}


tableau_poignet <- biometry %>%
  select(tour_poignet, masse_corr) %>%
  mutate(
    categorie_tour_poignet = cut(tour_poignet, 
                                 breaks = c(10, 12.5, 15, 17.5, 20, 22.5), 
                                 labels = c("10-12.5", "12.5-15", "15-17.5", "17.5-20", "20-22.5"))
  ) %>%
  group_by(categorie_tour_poignet) %>%
  summarise(
    moyenne_masse_corr = mean(masse_corr, na.rm = TRUE),
    nb_observations = n()
  )
  knitr::kable(tableau_poignet,caption = "Tableau : Moyenne de la Masse Réelle par Catégorie de Tour de Poignet", 
        col.names = c("Catégorie Tour de Poignet (cm)", "Moyenne Masse Réelle (kg)", "Nombre d'Observations"))
...
chart(biometry, tour_taille ~ tour_hanche) +
 geom_point()

```

   On peut observer une légère corrélation entre le tour de taille et le tour de hanche. Nous pouvons donc déduire que si un individu possède un tour de taille élevé auront tendance à avoir un tour de hanche élevé aussi.
...
```{r description2etudiant4}
 biometry %>.%
  ...
  knitr::kable(biometry_tab, rows = NULL, digits = 1, 
    col = c("Genre", "masse moyenne", "taille moyenne", "Observations"),
    caption = "Visualisation de la différence morphologique entre les hommes et les femmes")

```
   Ce tableau nous permet d'observer la différence morphologique entre les femmes et les hommes. On remarque que les hommes sont en moyenne plus grand et plus lourd que les femmes.
...
```{r plot_indice1}
chart(data = biometry, masse_corr ~ wwr | genre) + geom_point() +  xlab("rapport taille et poignet ") + ylab( "Masse de l'individu kg")
...
```{r plot_indice4}
# Graphique ou tableau utilisant l'indice calculé.
chart(biometry, masse_corr ~ bmi %col=% bmi_cat) +
  geom_point()+
...
```{r plot_t_2}
chart(data = biometry, wwr ~ bmi %col=% bmi_cat) +
  geom_abline(slope = 1, intercept = 0) +
  geom_point() +
  labs(x = "indice de masse corporel", y = "rapport tout de taille et tour de hanche")
