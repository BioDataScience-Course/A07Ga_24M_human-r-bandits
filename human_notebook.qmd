...
chart(biometry, alimentation ~ bmi) +
 geom_point()
...
biometry %<.%
  sselect(., sex, taille, masse)
  sgroup_by(., sex) %<.%
  smutate(., 
    taille_moy = fmean(taille),
    poid_moy = fmean(masse_corr),
    nbr = fn(sex)) -> c
tabularise(c)
...
b <- chart(biometry, IC ~ genre %fill=% genre) +
  geom_boxplot() +
  labs(y = "indice de conicité", x = "genre") +
