...
Nous observons sur ce graphique que la consommation de sucre ne semble pas avoir d'impact concret sur la masse des individu bien qu'il y ai plus de variation au plus les sujet ont tendance à en consommer souvent.

```{r description2etudiant3}
chart(data = biometry, masse_corr ~ activite_physique | genre) +
  geom_point() + labs( y = "Masse (kg)")


```

On observe une tendance à avoir une masse plus élevée lorsque l'activité physique est moindre. Au contraire, lorsque l'activité physique augmente, la masse est moins elévée.
...
<!--% Interprétation statistique et biologique -->
Les valeurs de p-value pour tout les termes sont supérieurs au seuil alpha à 5% (0.806, 0.280) ce qui veut dire qu'on ne rejette pas l'hypothèse nulle et qu'on a pas un effet significatif du genre ou des fumeurs sur le rtt.
Mais on observe une homocédaticité pour nos variables donc le test est pertinent.
